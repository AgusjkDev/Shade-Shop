        ...
        >
            {NAVBAR_ITEMS.map(({ key, className, onClick, ...props }) => (
                <Link
                    key={key}
                    className={cn(
                        "rounded-md text-center text-sm font-light uppercase tracking-wider text-foreground/80 transition-colors hover:text-foreground focus-visible:text-foreground focus-visible:ring-offset-4",
                        className,
                    )}
                    onClick={e => {
